...
set -a && source .env && set +a && go run -buildvcs=false ./src/product_service

# Or directly in the command
$(cat .env) go run -buildvcs=false ./src/product_service
```
...
- Swagger static files exist in `src/product_service/docs/`.
...
swag init -g src/product_service/main.go -o src/product_service/docs

# If "swag" is not in PATH, run it directly:
# $(go env GOPATH)/bin/swag init -g src/product_service/main.go -o src/product_service/docs
```

Swagger static files are deployed from `src/product_service/docs/` to the website bucket under `docs/`.
...
- A bucket deployment that uploads Swagger files from `src/product_service/docs/` to `docs/`
